...
def profile():
    ...
    try:
        events = db.events
        # ВИПРАВЛЕНО: використовуємо правильну колекцію day_entries (з модуля AI)
        ...
        # 2. Всього днів (кількість записів в щоденнику)
        total_days = day_entries.count_documents({"user_id": user_id})
  
        # 3. Середній настрій
        avg_mood_result = list(day_entries.aggregate([
        ...
        # 4. Стрік (днів підряд)
        streak_days = calculate_streak(user_id, day_entries)
        
        # Оновлюємо кеш статистики в документі користувача
        users.update_one(
            {"_id": user_id},
            {"$set": {
...
def calculate_streak(user_id, day_entries_collection):
    """
    Підрахунок кількості днів підряд.
    Виправлено назву поля на 'entry_date' (як в модулі AI).
    """
    try:
        # Отримуємо унікальні дати записів, відсортовані від нових до старих
        entries = list(day_entries_collection.find(
            {"user_id": user_id},
            {"entry_date": 1}
        ).sort("entry_date", -1))
        ...
        # Перетворюємо всі дати в об'єкти date (без часу) і беремо унікальні
        unique_dates = set()
        for entry in entries:
            ...
            if isinstance(d, datetime):
                ...
                try:
                    unique_dates.add(datetime.strptime(d[:10], "%Y-%m-%d").date())
                except:
                    pass
        
        # Сортуємо унікальні дати від нових до старих
        ...
        # Перевіряємо, чи є запис за сьогодні або вчора (щоб стрік не перервався)
        if sorted_dates[0] == today:
            ...
            # Якщо останній запис був давно, стрік обнуляється
            return 0
            
        # Рахуємо далі назад
        # Починаємо перевірку з 1-го індексу, якщо перший збігся
        start_index = 1
        
        for i in range(start_index, len(sorted_dates)):
            if sorted_dates[i] == current_check_date:
                streak += 1
